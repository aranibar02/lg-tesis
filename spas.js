...
var spaName = params[1];
var serviceUrl = '';
if(params.length > 1){
  serviceUrl = `https://ires2-tesis-backend.herokuapp.com/api/v1/spas?name=${spaName}`;
  option = 'Reservar'
}else{
  serviceUrl = `https://ires2-tesis-backend.herokuapp.com/api/v1/spas`;
...
fetch(serviceUrl)
  ...
  function render(data){
    ...
    var html = data.map(function(item,index){
        ...
        <button onclick="InfoRedirect()" class="btn btn-primary more-info">${option}</a>
        </div>
        </div>
        </div>
        </div>
